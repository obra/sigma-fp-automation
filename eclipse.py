...
def take_photo(camera, shutter_speed):
...
def take_one_photo(camera, shutter_speed):
    ...
    camera.set_cam_data_group3(CamDataGroup3(DestToSave=DestToSave.Both))
    camera.set_cam_data_group4(CamDataGroup4(ShutterSound=1))
    camera.set_cam_data_group1(CamDataGroup1(ShutterSpeed=ShutterSpeed2Converter.encode_uint8(shutter_speed), ISOSpeed=0x01, ISOAuto=0x00 ))
    # get_camera_data(camera)

    take_photo(camera, shutter_speed)
...
if __name__ == '__main__':
        ...
        while time.time() < end_time:
            ...
            with camera.session():
                ...
                take_one_photo(camera, 1/8000)
                take_one_photo(camera, 1/2000)
                take_one_photo(camera, 1/500)
                take_one_photo(camera, 1/125)
